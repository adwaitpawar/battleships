...
ZIP_FILE="battleships.zip"  # Path to the ZIP file to upload

# Authenticate with Nexus repository and upload ZIP file
curl -u "$NEXUS_USERNAME:$NEXUS_PASSWORD" -X POST "$REPOSITORY_URL" -H "accept: application/json" -H "Content-Type: multipart/form-data" -F "artifact=@$ZIP_FILE;type=application/zip"
